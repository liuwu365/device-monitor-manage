...
public class BackUserServiceImpl implements BackUserService {
    ...
    @Override
    public BackUser getItemByAccountName(String accountName) {

        return this.backUserMapper.selectByAccountName(accountName);
    }
    ...
    @Override
    public List<BackUserLogin> getLoginInfoList(Map<String, Object> map) {
        return backUserLoginMapper.selectLoginInfo(map);
    }
    ...
    public Result delItem(Long id) {
        ...
        try {
            ...
            backUserRoleMapper.delByUserId(id);
            backResRoleMapper.delByUserId(id);
            backUserLoginMapper.deleteByUid(id);
    ...
    private boolean authen(BackUser user, String password) {
        ...
        SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(
                ...
                "com.device.manage.core.auth.ShiroRealmImpl_0" // realm name
        );
        return matcher.doCredentialsMatch(token, authenticationInfo);
    }

    @Override
    public boolean pathsMatch(String path, ServletRequest request) {
        try {
            BackIpLimit backIpLimit = backIpLimitMapper.selectByIpAddress(path);
            if (!CheckUtil.isEmpty(backIpLimit)) {
                return true;
            } else {
                return false;
            }
        } catch (Exception e) {
            LOGGER.error("pathsMatch error|path={}|ex={}", path, ErrorWriterUtil.writeError(e).toString());
            throw new RuntimeException();
        }
    }
}
